    ...
    create_payment = async (req, res) => {
        ...
        try {
            ...
            const order = await razorpay.orders.create({
                amount: price * 100, // Convert amount to paise (Razorpay requires this format)
                currency: 'INR', // Replace with your desired currency (e.g., USD, EUR, etc.)
            });
    
           
            responseReturn(res, 200, { orderId: order.id });
        } catch (error) {
            console.log(error.message);
            responseReturn(res, 500, { error: 'Failed to create payment' });
        }
    };
